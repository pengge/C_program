...
HRESULT CDRM::Init( CAudioPlay* pParent, IWMReader* pReader, LPCWSTR pwszURL )
{
	...
	if( FAILED( hr ) )
	{
		...
		wsprintf( m_tszErrMsg, _T( "Could not QI for IWMDRMReader (hr=%#X)" ), hr );
	...
	case WMT_LICENSEURL_SIGNATURE_STATE:
			...
			switch((int)*pValue)
			{
				...
				case WMT_DRMLA_TAMPERED:
						dwContinue = MessageBox( NULL, _T( "The license acquisition URL in this file has been tampered with. You are strongly advised not to navigate to this Web site. Navigate to this Web site anyway?" ),
                          						ERROR_DIALOG_TITLE, MB_YESNO );
					break;
				default:
					dwContinue = MessageBox( NULL, _T( "The license acquisition URL in this file is not signed and its validity cannot be guaranteed. Get a license anyway?" ),
                 						ERROR_DIALOG_TITLE, MB_YESNO );
					break;
			}
...
HRESULT CDRM::GetNonSilentLicense( BYTE *pValue )
{
	...
	ZeroMemory( ( void* )m_tszErrMsg, sizeof( m_tszErrMsg ) );
	...
	do
	{
		...
		ptszURL = new TCHAR[ _tcslen( ptszEscapedURL ) + wcslen( ( LPWSTR )pValue ) + 30 ];
		if( NULL == ptszURL )
		{
        ...
        //
		wsprintf( ptszURL, _T( "%ws&filename=%s&embedded=false" ), ( LPWSTR )pValue, ptszEscapedURL );
        ...
        if( FAILED( hr ) )
        {
            wsprintf( m_tszErrMsg, _T( "Unable to launch the URL (Err=%#X)" ), hr );
			break;
        }
    ...
    SAFE_ARRAYDELETE(ptszEscapedURL);
	SAFE_ARRAYDELETE(ptszURL);

	if( FAILED( hr ) && ( _tcslen( m_tszErrMsg ) > 0 ) )
	{
		//MessageBox( NULL, m_tszErrMsg, ERROR_DIALOG_TITLE, MB_OK );
	}
...
HRESULT CDRM::MakeEscapedURL( LPCWSTR pwszInURL, LPTSTR *ppszOutURL )
{
    ...
    *ppszOutURL = new TCHAR[ cchOutURL ];
    if( NULL == *ppszOutURL )
    {
		//MessageBox( NULL, _T( "Insufficient memory" ), ERROR_DIALOG_TITLE, MB_OK );
    ...
    while( TRUE )
    {
        ...
        if( NULL == pchToEscape )
        {
            ...
            //
            wsprintf( pchNext, _T( "%ws" ), pwszTemp );
            break;
        }
		...
		*pchToEscape = L'\0';
        ...
        //
        pchNext += wsprintf( pchNext, _T( "%%%02x" ), chEscape );
...
HRESULT CDRM::LaunchURL( LPCTSTR ptszURL )
{
    ...
    do
    {
        ...
        if( NULL != ptszParam )
        {
            ...
            *ptszParam = _T( '\0' );
            wsprintf( ptszLaunchCommand, _T( "%s%s%s" ), tszShellOpenCommand, ptszURL, ptszParam + 4 );
        }
        else
        {
            //
            ...
            wsprintf( ptszLaunchCommand, _T( "%s %s" ), tszShellOpenCommand, ptszURL );
        }
                                   ...
                                   LPTSTR ptszApplicationName )
{
    ...
    _tcsncpy( ptszApplicationName, pchFirst, pchNext - pchFirst );
...
HRESULT CDRM::CopyWMGetLicenseData( WM_GET_LICENSE_DATA* pOriginalData, WM_GET_LICENSE_DATA* pDestData, DWORD* pcbDestSize )
{
    ...
    if ( pDestData )
    {
        if ( *pcbDestSize >= pcbRequiredSize )
        {
            ...
            wcscpy( pDestData->wszURL, pOriginalData->wszURL );
            wcscpy( pDestData->wszLocalFilename, pOriginalData->wszLocalFilename );
