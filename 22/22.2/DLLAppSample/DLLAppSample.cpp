// DLLAppSample.cpp : Defines the class behaviors for the application.
//

#include "stdafx.h"
#include "DLLAppSample.h"
#include "DLLAppSampleDlg.h"

#ifdef _DEBUG
#define new DEBUG_NEW
#undef THIS_FILE
static char THIS_FILE[] = __FILE__;
#endif

/////////////////////////////////////////////////////////////////////////////
// CDLLAppSampleApp

BEGIN_MESSAGE_MAP(CDLLAppSampleApp, CWinApp)
	//{{AFX_MSG_MAP(CDLLAppSampleApp)
		// NOTE - the ClassWizard will add and remove mapping macros here.
		//    DO NOT EDIT what you see in these blocks of generated code!
	//}}AFX_MSG
	ON_COMMAND(ID_HELP, CWinApp::OnHelp)
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CDLLAppSampleApp construction

CDLLAppSampleApp::CDLLAppSampleApp()
{
	// TODO: add construction code here,
	// Place all significant initialization in InitInstance
}

/////////////////////////////////////////////////////////////////////////////
// The one and only CDLLAppSampleApp object

CDLLAppSampleApp theApp;

/////////////////////////////////////////////////////////////////////////////
// CDLLAppSampleApp initialization

BOOL CDLLAppSampleApp::InitInstance()
{
	AfxEnableControlContainer();

	// Standard initialization
	// If you are not using these features and wish to reduce the size
	//  of your final executable, you should remove from the following
	//  the specific initialization routines you do not need.

#ifdef _AFXDLL
	Enable3dControls();			// Call this when using MFC in a shared DLL
#else
	Enable3dControlsStatic();	// Call this when linking to MFC statically
#endif

	CDLLAppSampleDlg dlg;
	m_pMainWnd = &dlg;
	int nResponse = dlg.DoModal();
	if (nResponse == IDOK)
	{
		// TODO: Place code here to handle when the dialog is
		//  dismissed with OK
	}
	else if (nResponse == IDCANCEL)
	{
		// TODO: Place code here to handle when the dialog is
		//  dismissed with Cancel
	}

	// Since the dialog has been closed, return FALSE so that we exit the
	//  application, rather than start the application's message pump.
	return FALSE;
}

BOOL CALLBACK EnumIconProcedure(HANDLE hModule, LPCTSTR lpszType, LPTSTR lpszName, LONG lParam)
{
	HICON icon = LoadIcon((HINSTANCE)hModule,lpszName);
	SendMessage((HWND)lParam, WM_ICON_MESSAGE, (LPARAM)icon, (WPARAM)lpszName);	
	return TRUE;
}

BOOL CALLBACK EnumBitmapProcedure(HANDLE hModule, LPCTSTR lpszType, LPTSTR lpszName, LONG lParam)
{
	HBITMAP bitmap = LoadBitmap((HINSTANCE)hModule,lpszName);
	if (true)
	{
		SendMessage((HWND)lParam, WM_BITMAP_MESSAGE, (LPARAM)bitmap, (WPARAM)lpszName);	
		return FALSE;
	}
}

BOOL CALLBACK EnumDialogProcedure(HANDLE hModule, LPCTSTR lpszType, LPTSTR lpszName, LONG lParam)
{
	//DWORD dwID=1547;
	HRSRC hRes = FindResource((HINSTANCE)hModule, lpszName, RT_DIALOG) ;      
	HWND hDialog = (HWND)LoadResource((HINSTANCE)hModule, hRes) ; 
	if (hDialog)
	{
	::ShowWindow(hDialog, SW_SHOW);     
	::UpdateWindow(hDialog); 
	}
	//SendMessage((HWND)lParam, WM_DIALOG_MESSAGE, (LPARAM)NULL, (WPARAM)lpszName);
	
	return TRUE;
}

